...
for name in [1,2,3,4,5,6,7,8,9]:
    ...
    try:
        in_dir = '/home/jerichoo/projects/rrg-fherwig-ad/fherwig/PPM_unprocessed/A/PPM_A{}'.format(name)
        out_dir = '/home/jerichoo/projects/rrg-fherwig-ad/fherwig/PPM_processed/A/pPPM_A{}' .format(name)

        info = reformation_info(in_dir,out_dir,all_files=True,max_dumps=None)

        reformat_parallel(info,out_dir,all_files = True)
    except:
        print 'no reformatting done for A {}'.format(name)
