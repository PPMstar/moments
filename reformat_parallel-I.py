        ...
        if ftype in RBO_key:
            ...
            if bob2hv_ftype is not None:                
                ...
                if any([ftype in type for type in info.is_hires]):
                    resolutionx *= info.source_code_definitions['nnxteam']
                    resolutiony *= info.source_code_definitions['nnyteam'] 
                    RBO_file = info.hv_processing_format.format(ftype=ftype, RBO_in_ftype=bob2hv_ftype, dump=dump, ext=".bob8aaa")
                else:
                    resolutionz /= info.source_code_definitions['nnzteam'] 
                ...
                bob2hv_command = [info.bob2hv_path, str(resolutionx), str(resolutiony), str(int(resolutionz)),RBO_file, "-t",
                                  ...
                                  str(2*info.source_code_definitions["nnzteams"]), "-s", "128"]
                
                bob2hv = subprocess.Popen(bob2hv_command, cwd=processing_dir)
                bob2hv.wait()
                print ' '.join(RBO_command)
...
class reformation_info(object):
    ...
    def __init__(self,source_dir,target_dir,all_files=False,max_dumps=None):
        ...
        self.RBO_source = pkg_resources.resource_string("moments.utils", "/bin/ReformatBigOutputargs2.F")
...
#for name in ['13_uncompressed','5_uncompressed',3,13]:
    
for name in ['768_Burn','Burn_enh','PPM-sakurai','PPM-star-no-burn_heatingx5','768_Qzero','Fall2013','PPM-star-no-burn_heatingx10','PPM-star-no-burn_heatx10_Th0.6trans_3.2Th1.2e1.0']:
        try:
            in_dir = '/home/jerichoo/projects/rrg-fherwig-ad/fherwig/PPM_unprocessed/Sakurai/{}'.format(name)
            out_dir = '/home/jerichoo/projects/rrg-fherwig-ad/fherwig/PPM_processed/Sakurai/p{}'.format(name)

            info = reformation_info(in_dir,out_dir,all_files=True,max_dumps=None)

            reformat_parallel(info,out_dir,all_files = True)
        except:
            print 'no reformatting done for I {}'.format(name)
