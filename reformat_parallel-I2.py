...
def reformat_hv(ftype,dump,info):
    ...
    for file in info.source.get_dumpfiles(ftype, dump):
        ...
        if RBO_in_ftype is not None:
            ...
            if ftype == 'FV-hires':
                nbytes_theoretical *= 8 # don't know if this is true
    ...
    for RBO_key in info.RBO_input_map.keys():         
        if ftype in RBO_key:
            ...
            #making sure that HV-processing exists
            out_dir = os.path.dirname(info.hv_source_format.format(ftype=ftype,dump=dump))
            if not os.path.exists(out_dir):
                os.makedirs(out_dir)
            ...
            if bob2hv_ftype is not None:                
                resolutionx = info.source_code_definitions["nntxbricks"]*info.source_code_definitions["nnnnx"]
                resolutiony = info.source_code_definitions["nntybricks"]*info.source_code_definitions["nnnny"]
                resolutionz = info.source_code_definitions["nntzbricks"]*info.source_code_definitions["nnnnz"]
                if any([ftype in type for type in info.is_hires]):
                    resolutionx *= 2 #info.source_code_definitions["nnxteams"]
                    resolutiony *= 2 #info.source_code_definitions["nnyteams"]
                    #resolutionz *= 2 #info.source_code_definitions["nnzteams"]
                ...
                # don't know why there is the -s paramater
                bob2hv_command = [info.bob2hv_path, str(resolutionx), str(resolutiony), str(int(resolutionz)),RBO_file, "-t",
                                  str(info.source_code_definitions["nnxteams"]), str(info.source_code_definitions["nnyteams"]),\
                                  str(2*info.source_code_definitions["nnzteams"]), "-s", "128"] # cahnged to 2
                print ' '.join(bob2hv_command)
                ...
                try:
                    ...
                    log.error('One of: '+ ' '.join(RBO_command)+ ' or ' + ' '.join(bob2hv_command) + ' failed')
...
class reformation_info(object):
    ...
    def __init__(self,source_dir,target_dir,all_files=False,max_dumps=None):
        ...
        self.RBO_source = pkg_resources.resource_string("moments.utils", "/bin/ReformatBigOutputargs2.F") #ReformatBigOutputargs2.F is script for 2 new z per old
        ...
        if 'source_code' not in locals():
            source_code = self.source.get_source_code()[0] # should be zero this is a hacky change for J2
        
        #source_code = '/home/jerichoo/projects/rrg-fherwig-ad/fherwig/PPM_unprocessed/I/PPM_I2/PPM2F-star_I2.F'
        log.info('%s will be used as the source file' % (source_code))   
...
def reformat_parallel(info,target_dir,all_files=False,max_dumps=None):
    ...
    Parallel(n_jobs=num_cores)(
        ...
        for ndump in info.source.get_dumps(ftype)) #in [0001])
...
'''
for name in ['4_uncompressed','6_uncompressed']:
    
    
        in_dir = '/home/jerichoo/projects/rrg-fherwig-ad/fherwig/PPM_unprocessed/J/PPM_J{}'.format(name)
        out_dir = '/home/jerichoo/projects/rrg-fherwig-ad/fherwig/PPM_processed/J/pPPM_J{}' .format(name)
